...
if (isset($_POST['borrow'])) {
    if (!empty($_POST['title'])) {
        ...
        if ($check && mysqli_num_rows($check) > 0) {
            ...
            $dup = mysqli_query($conn, "SELECT * FROM borrowed_books WHERE username='$username' AND book_title='$title'");
            if (mysqli_num_rows($dup) > 0) {
                echo "<script>alert('You already borrowed this book!');</script>";
            } 
            else if ($book['quantity'] > 0) {
                // Insert borrow record
                mysqli_query($conn, "INSERT INTO borrowed_books (username, book_title, borrow_date)
...
.card { background: rgba(255,255,255,0.9); padding: 20px; border-radius: 15px; box-shadow: 0 5px 20px rgba(0,0,0,0.3); transition: 0.3s; }
.card:hover { transform: translateY(-5px); }
.card h3 { margin: 0 0 10px; color: #4a148c; }
.card p { margin: 5px 0; font-size: 14px; }
.borrow-btn { margin-top: 10px; padding: 8px 12px; background: #6a1b9a; color: white; border: none; border-radius: 6px; cursor: pointer; }
.borrow-btn:hover { background: #4a148c; }
.not-available { margin-top: 10px; color: red; font-weight: bold; }
.back-btn { display: inline-block; margin: 20px; text-decoration: none; background: white; color: #4a148c; padding: 10px 15px; border-radius: 8px; }
